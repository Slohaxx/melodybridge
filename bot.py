...
server = "35.225.214.145" # Server
channel = "#main" # Channel
botnick = "cloudapp" # Your bots nick
password = "secret"
lines = 0
#regexes = [".*"]
...
def sed(msg):
  ...
  # loop through the chat log and search through the messages for a match to the 'find' string. Ignore an     y errors -- to be fixed.
  for i in content:
    try:
  ...
  # if the default replaced text was found, perform a replace on the text, strip any newlines, and send t     o checksend method for verification before sending to channel.
...
def regex(msg):
 ...
 # if there is no text between the first and last '|' (as in 's||somethin') do nothing because there is      nothing to find.
  ...
  # set the default replaced text to a known, default value
  replaced = ".__not found__."
  # escape any '*' wildcards as they are too broad so assume they are literal. Needs testing for '\*' sea     rches.
  if (sedtest.find("*") != -1):
  ...
  for i in content:
    try:
      ...
      text = i.split(":",1)[1]
      # search the text for regex pattern, if found, set found equal to the text and replace it with repl     ace string. Set name equal to the username of found message.
    ...
    # ignore errors and continue. --Needs further testing
      ...
      pass
  # if the regex found more than 10 matches, assume it was too broad and let the user know. --Change to w     hisper?
  if repltext[1] > 10:
    sendmsg("Too many matches, refine your correction")
    return
  # set replaced string equal to just the new message and strip newlines, then send to checksend for veri     fication before sending to channel.
  replaced = repltext[0]
  replaced = replaced.strip('\n\r')
...
def checksend(name,orig,new,pattern=''):
  ...
  if orig == new:
    ...
    return
  # if the find used very broad wildcards then do nothing. May need to add more or think of new method fo     r this.
...
def help(name,topic=''):
  ...
  if topic == '':
    message = "Hi! I am an irc bot created by OrderChaos. Currentl function is find and replace. You can      perform a plain text find/replace on recent chat by typing in \'s/[find]/[replace]\' (no quotes) or a reg     ex based find/replace with \'s|[find]|[replace]\'. Note that some wildcards are disabled because they res     ult in too many matches. Please report any bugs or suggestions here: https://github.com/Orderchaos/ircbot     /issues"
